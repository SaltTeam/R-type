...
namespace Entities {
    ...
    public:
        TieExperiment(SCOPE *scope, uint64_t id = 0, network::protocol::Update updateType = network::protocol::Update::Replica, uint16_t refreshTime = 1000,
                      bool isEnabled = true, const float &x = 0, const float &y = 0)
                : Ship(scope, id, updateType, refreshTime, isEnabled, Entities::Ship::GAME, x, y, 0.15, 0.15, 40) {
        ...
        void update() override {
            ...
            if (this->position.x < 10 + this->texture->sprite.getGlobalBounds().width)
                this->speed.x = 0.15;
            if (this->position.x + this->texture->sprite.getGlobalBounds().width > this->scope->gameService->getWindowSize().x - 50)
                this->speed.x = -0.15;
            if (this->position.y < 10 + this->texture->sprite.getGlobalBounds().height)
                this->speed.y = 0.15;
            if (this->position.y + this->texture->sprite.getGlobalBounds().height > this->scope->gameService->getWindowSize().y - 50)
                this->speed.y = -0.15;
